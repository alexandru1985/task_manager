...
class ImportCSVController extends Controller
{
    ...
    public function saveCSVtoDb(Request $request)
    {
        ...
        if($validator){
            return redirect('import-csv')->withErrors($validator);
        }
        else {
            ...
            foreach($data as $key => $rows) {
                ...
                if (is_string($rows[2])) {
                    $tasks = Tasks::firstOrNew([
                        ...
                        'project_id' => $this->findProjectId(trim($rows[1])),
                        'name' => trim($rows[2])
                    ]);
                    $tasks->save();
                }
                ...
                if (is_string($rows[3])) {
                    $users = explode(",", $rows[3]);
                    $roles = explode(",", $rows[4]);
                    $projectId = $this->findProjectId(trim($rows[1]));
                    foreach($users as $key => $user) {
                        ...
                        $userTasks = UserTasks::firstOrNew([
                            ...
                            'task_id' => $this->findTaskId(trim($rows[2]), $projectId)
                        ]);
                        $userTasks->save();
                    }
                }         
            }
            return view('message-csv');
        }
    }
    ...
    public function validateInputs(Request $request) {
        ...
        $validator->after(function ($validator) use ($request){
            if($this->checkExtension($request->file('file_csv')->getClientOriginalExtension()) == false) {
